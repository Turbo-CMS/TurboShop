...
$filter['visible'] = 1;
$rss = $turbo->articles->get_articles($filter);

foreach($rss as $r) {
	print '<item turbo="true">';
		print '<title>'.htmlspecialchars($r->name).'</title>';
		print '<link>'.$turbo->config->root_url.'/article/'.$r->url.'</link>';
		print '<turbo:topic>'.htmlspecialchars($r->name).'</turbo:topic>';
        print '<turbo:source>'.$turbo->config->root_url.'/article/'.$r->url.'</turbo:source>';
        print '<pubDate>'.$r->last_modified.'</pubDate>'; // D, d M Y H:i:s +0000
		print '<turbo:content><![CDATA[';
		
		// Display image
		if(!empty($r->image)){
			print '<figure><img src="'.$turbo->design->resize_articles_modifier($r->image, 400, 400, false).'"/></figure>';
		}	
		...
		if(!empty($r->text)){
			print '<button formaction="'.$turbo->config->root_url.'/article/'.$r->url.'" data-background-color="#0893b9" data-color="#ffffff" data-turbo="false" data-primary="true" >Полная версия ...</button>';
   		}
