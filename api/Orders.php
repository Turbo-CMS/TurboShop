...
class Orders extends Turbo
{
	...
	function get_orders($filter = array())
	{
		...
		if (!empty($filter['from_date']) || !empty($filter['to_date'])) {
			if (!empty($filter['from_date'])) {
				...
				$from = '1970-01-01'; /*if there is no start date, take the time since the UNIX epoch*/
			}
			if (!empty($filter['to_date'])) {
				$to = date('Y-m-d', strtotime($filter['to_date']));
			} else {
				$to = date('Y-m-d'); /*if there is no end date, take "today" as the date*/
			}
		...
		$query = $this->db->placehold("SELECT o.id, o.delivery_id, o.delivery_price, o.separate_delivery,
									...
									WHERE 1
									$id_filter $status_filter $user_filter $keyword_filter $label_filter $date_filter $modified_since_filter GROUP BY o.id ORDER BY status, id DESC $sql_limit", "%Y-%m-%d");
		$this->db->query($query);
		$orders = array();
	...
	function count_orders($filter = array())
	{
		...
		$query = $this->db->placehold("SELECT COUNT(DISTINCT id) as count
									...
									LEFT JOIN __orders_labels AS ol ON o.id=ol.order_id 
									WHERE 1
									$status_filter $user_filter $label_filter $date_filter $keyword_filter");
		$this->db->query($query);
	...
	*/
	...
	public function update_purchase($id, $purchase)
	{
		...
		// If the order is closed, you need to update the warehouse when changing the purchase
	...
	public function add_purchase($purchase)
	{
		...
		// If the order is closed, you need to update the warehouse when adding a purchase
