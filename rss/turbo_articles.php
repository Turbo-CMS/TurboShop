...
$filter['visible'] = 1;
$rss = $turbo->articles->getArticles($filter);
...
foreach ($rss as $r) {
	...
	echo '<link>' . $turbo->config->root_url . '/' . $langLink . 'article/' . $r->url . '</link>';
	echo '<turbo:topic>' . htmlspecialchars($r->name) . '</turbo:topic>';
	echo '<turbo:source>' . $turbo->config->root_url . '/' . $langLink . 'article/' . $r->url . '</turbo:source>';
	echo '<pubDate>' . $r->last_modified . '</pubDate>';
	echo '<turbo:content><![CDATA[';

	if (!empty($r->image)) {
		echo '<figure><img src="' . $turbo->design->resizeArticlesModifier($r->image, 400, 400, false) . '"/></figure>';
	}
	...
	if (!empty($r->text)) {
		echo '<button formaction="' . $turbo->config->root_url . '/' . $langLink . 'article/' . $r->url . '" data-background-color="#0893b9" data-color="#ffffff" data-turbo="false" data-primary="true">&#129122;</button>';
	}
