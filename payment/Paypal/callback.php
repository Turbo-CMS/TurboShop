...
// Get the order
$order = $turbo->orders->get_order(intval($turbo->request->post('invoice')));
if(empty($order))
	die('Order not found');
...
{
	$delivery_price = $turbo->money->convert($order->delivery_price, $payment_method->currency_id, false);
	$delivery_price =round($delivery_price, 2);
	$total_price += $delivery_price;
}
if($total_price != $turbo->request->post('mc_gross'))
	die("Incorrect total price (".$total_price."!=".$turbo->request->post('mc_gross').")");
