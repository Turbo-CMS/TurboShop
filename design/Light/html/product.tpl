                ...
                {if $product->rating > 0}
                    <div class="label-rating">
                        <span class="test-text">
                            <span itemprop="ratingValue" class="rater-rating">{$product->rating|string_format:"%.1f"}</span>&#160;(<span itemprop="reviewCount" class="rater-rateCount">{$product->votes|string_format:"%.0f"} {$lang->votes}</span>)
                                ...
                                <tr {if $product->colors|count < 2} style="display:none;" {/if}>
                                    <td id="colors">
                                        ...
                                        {foreach $product->colors as $k=>$v}
											<label title="{$k}" {if $v['code']}style="background: {$v['code']}"{/if} {if $v@first}class="active"{/if}><input type="radio" name="color" value="{$k}" {if $v@first}checked{/if} />{if $v['code']}&nbsp;&nbsp;&nbsp;&nbsp;{else}{$k}{/if}</label>
                                        {/foreach}
                                    </td>
                                </tr>
                                <tr {if !$product->variant->name} style="display:none;" {/if}>
                                    <td id="variants">
                                        ...
                                        {foreach $product->variants as $v}
											<label {if $v@first}class="active"{/if}><input type="radio" name="variant" value="{$v->id}" {if $v@first}checked{/if} />{$v->name}</label>
                                        {/foreach}
                                    </td>
                                </tr>
                            </table>
