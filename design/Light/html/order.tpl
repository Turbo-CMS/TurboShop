    ...
    {if !$order->paid}
        ...
        {if $payment_methods && !$payment_method && $order->total_price>0}
            <form method="post">
                ...
                <input type="submit" class="btn btn-success btn-lg float-right" value="{$lang->finish_the_order}">
            </form>
            {* Selected payment method *}
        {elseif $payment_method}
            <h2>{$lang->payment_method} &mdash; {$payment_method->name}</h2>
            <form method="post"><input type="submit" class="btn btn-primary" name="reset_payment_method" value="{$lang->choose_another_payment_method}"></form>
            <p>
                {$payment_method->description}
            </p>
            <h2>
