            ...
            <div class="row">
                ...
                <aside class="col-sm-3 border-left">
                    <div class="action-wrap">
                        ...
                        {/if}
                        {if $wishlist}
                            <a href="{$lang_link}wishlist/remove/{$product->url}"><i class="fa fa-heart text-danger"></i> {$lang->delete}</a>
                        {elseif $wishlist_products && in_array($product->url, $wishlist_products)}
                            <a href="{$lang_link}wishlist"><i class="fa fa-heart text-danger"></i> {$lang->added_to_wishlist}</a>
                        {else}
                            <a class="wishlist-list" data-wishlist="<i class='fa fa-heart text-danger'></i> {$lang->added_to_wishlist}" href="wishlist/{$product->url}"><i class="fa fa-heart text-secondary"></i> {$lang->add_to_wishlist}</a>
                        {/if}
                        </br>
                        {if $smarty.session.compared_products && in_array($product->url, $smarty.session.compared_products)}
                            <a href="{$lang_link}compare"><i class="fa fa-chart-bar text-primary"></i> {$lang->added_to_compare}</a>
                        {else}
                            <a class="compare-list" href="compare/{$product->url}" data-compare="<i class='fa fa-chart-bar text-primary'></i> {$lang->added_to_compare}"><i class="fa fa-chart-bar text-secondary"></i> {$lang->add_to_compare}</a>
                        {/if}
                    </div>
                </aside>
            </div>
