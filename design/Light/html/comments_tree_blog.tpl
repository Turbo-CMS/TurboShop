{if $comments}
    <div class="btn-toolbar justify-content-between mb-4" role="toolbar" aria-label="Toolbar with button groups">
        <span>{$comments_count} {$lang->comments_global}</span>
    ...
    {function name=comments_tree}
        {foreach $comments as $comment}
            ...
            <div class="media comments my-4">
                {if $comment->admin == 1}
                    <img class="d-flex mr-3" src="design/{$settings->theme|escape}/images/admin.jpg" alt="{$comment->name|escape}">
                {else}
                    <img class="d-flex mr-3" src="design/{$settings->theme|escape}/images/user.jpg" alt="{$comment->name|escape}">
                {/if}
