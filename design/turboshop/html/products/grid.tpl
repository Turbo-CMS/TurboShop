<div class="catalog-block__wrapper grid-list__item grid-list-border-outer {if !$recommended_products}js-infinite{/if} product {if $recommended_products}swiper-slide swiper-slide--height-auto{/if} {if (isset($product->related_products) && $product->related_products || $product->variants|count > 1) && !$recommended_products}has-offers{/if}" data-hovered="false">
	<div class="js-catalog__wrapper catalog-block__item outer-rounded-x bg-theme-parent-hover border-theme-parent-hover color-theme-parent-all js-popup-block {if $recommended_products}bordered{else}shadow-hovered shadow-hovered-f600 shadow-no-border-hovered {if $theme_settings->products_grid_bordered}bordered{/if}{/if}">
		<div class="catalog-block__inner flexbox height-100" itemprop="itemListElement" itemscope="" itemtype="http://schema.org/Product">
					...
					<a href="{$lang_link}products/{$product->url}" class="image-list__link">
						<span class="section-gallery-wrapper js-replace-gallery flexbox image">
							{if $product->images|count > 1 && $theme_settings->products_img_gallery && $theme_settings->products_img_gallery_limit > 1}
								{foreach $product->images|@array_slice:0:$theme_settings->products_img_gallery_limit as $i=>$image name=img}
			...
			<div class="catalog-block__info-bottom  {if (isset($product->related_products) && $product->related_products || $product->variants|count > 1) && !$recommended_products}catalog-block__info-bottom--with-sku{/if}">
				<form class="variants" action="{$lang_link}cart">
					...
					{if (isset($product->related_products) && $product->related_products || $product->variants|count > 1) && !$recommended_products}
