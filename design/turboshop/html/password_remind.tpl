					...
					<div class="breadcrumbs swipeignore" itemscope="" itemtype="http://schema.org/BreadcrumbList">
						...
						{else}
							<span class="breadcrumbs__item" itemprop="itemListElement" itemscope="" itemtype="http://schema.org/ListItem">
								...
								<span>
									<span itemprop="name" class="breadcrumbs__item-name font_13">{$lang->password_recovery}</span>
									<meta itemprop="position" content="{$level++}">
								</span>
							</span>
						{/if}
			...
			<div class="topic">
				<div class="topic__inner">
					<div class="topic__heading">
						...
						{else}
							<h1 id="pagetitle" class="switcher-title">{$lang->password_recovery}</h1>
						{/if}
					</div>
				</div>
			</div>
						...
						<div class="form">
							...
							<form id="forgotpasswd-page-form" class="validate" method="post" novalidate="novalidate">
								<div class="top-text font_16">
									{$lang->password_recovery_text}
								</div>
								<div class="form-body">
									<div class="form-group fill-animate">
										<label for="FORGOTPASSWD_USER_LOGIN" class="font_14"><span>Email:&nbsp;<span class="required-star">*</span></span></label>
										<div class="input">
											<input id="FORGOTPASSWD_USER_LOGIN" class="form-control required" type="text" name="email" value="{if isset($email)}{$email|escape}{/if}" required="" maxlength="255" autocomplete="off" aria-required="true">
										</div>
										<div class="text_block font_13">{$lang->recovery_text}</div>
									</div>
								</div>
								<div class="form-footer">
									<button class="btn btn-default btn-lg btn-wide has-ripple" type="submit" name="login" value="{$lang->restore}"><span>{$lang->restore}</span></button>
