...
<div itemscope itemtype="http://schema.org/Product">
        ...
        <div class="col-md-8">
            ...
            {if $product->images|count>1}
                <span class="d-sm-none d-md-block d-none">
                    <div id="product-slider-pagination" class="row text-center text-lg-left {if $product->colors|count > 1}loader-container{/if}" {if $product->colors|count > 1}style="display: none;" {/if}>
            ...
            {if $product->variants|count > 0}
                <div itemprop="offers" itemscope="" itemtype="http://schema.org/Offer">
                    ...
                    <div class="btn-group ml-3" role="group" aria-label="First group">
                        ...
                        <a class="btn-lg btn-light mr-1" href='wishlist'><i class="fal fa-heart text-danger"></i></a>
                        {else}
                        <a class="btn-lg btn-light wishlist mr-1" href='wishlist/{$product->url}'><i class="fal fa-heart text-secondary"></i></a>
                        {/if}
                        {if $smarty.session.compared_products && in_array($product->url, $smarty.session.compared_products)}
                        <a class="btn-lg btn-light" href='compare'><i class="fal fa-sync text-primary"></i></a>
                        {else}
                        <a class="btn-lg btn-light compare" href='compare/{$product->url}'><i class="fal fa-sync text-secondary"></i></a>
                        {/if}
                    </div>
                    <a class="btn btn-success btn-lg mt-3 fast-order-send-button" href="javascript:void(0)" role="button" data-name="{$product->name}" data-toggle="modal" data-target="#fast_order"><i class="fal fa-shopping-bag"></i> {$lang->buy_one_click}</a>
                </form>
    ...
    <ul class="nav nav-tabs" id="myTab" role="tablist">
        ...
        {/if}
        <li class="nav-item">
            <a class="nav-link" id="comments-tab" data-toggle="tab" href="#comments" role="tab" aria-controls="comments" aria-selected="false">{$comments_count|plural:$lang->comment_1:$lang->comment_2:$lang->comment_3} ({$comments|count})</a>
        </li>
        {if $cms_files}
        <li class="nav-item">
