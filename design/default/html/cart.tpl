...
<div class="row my-4">
	...
	{if $cart->purchases}
		<div class="col-lg-12 mt-2">
			<div class="float-end">
				<a class="btn btn-primary btn-sm" href="{url empty=1}"><i class="fal fa-trash"></i> <span>{$lang->empty_trash}</span></a>
			</div>
		</div>
	{/if}
</div>
<fieldset>
	{if $cart->purchases}
				...
				<tbody>
					{foreach $cart->purchases as $purchase}
						<tr>
							...
							<td data-title="{$lang->delete}"><a href="{$lang_link}cart/remove/{$purchase->variant->id}" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="{$lang->delete}"><i class="fal fa-trash text-muted"></i></a></td>
						</tr>
					{/foreach}
				</tbody>
			</table>
			...
			<div class="row">
				<div class="col-lg-6">
					<div class="input-group mb-3">
						<input class="form-control" placeholder="{$lang->enter_coupon_code}" type="text" name="coupon_code" value="{if isset($cart->coupon->code)}{$cart->coupon->code|escape}{/if}">
								...
								<h5 class="mb-0">
									<div class="form-check">
										<label class="form-check-label">
											<input type="radio" class="form-check-input {if $delivery@first}collapsed{/if}" name="delivery_id" value="{$delivery->id}" data-bs-toggle="collapse" data-bs-target="#collapseDelivery{$delivery->id}" aria-expanded="true" aria-controls="collapseDelivery{$delivery->id}" {if isset($delivery_id) && $delivery_id==$delivery->id}checked{elseif $delivery@first}checked{/if} id="deliveries_{$delivery->id}">
											{$delivery->name}
						...
						<div class="card my-2">
							<div class="card-header">
								<h5 class="mb-0">
									<div class="form-check">
										<label class="form-check-label">
											<input type="radio" class="form-check-input {if $payment_method@first}collapsed{/if}" name="payment_method_id" value="{$payment_method->id}" data-bs-toggle="collapse" data-bs-target="#collapsePayment{$payment_method->id}" aria-expanded="true" aria-controls="collapsePayment{$payment_method->id}" {if $payment_method@first}checked{/if} id="deliveries_{$payment_method->id}">
											{$payment_method->name}, {$lang->to_pay_small} {$cart->total_price|convert:$payment_method->currency_id}&nbsp;{$all_currencies[$payment_method->currency_id]->sign}
										</label>
									</div>
								</h5>
							</div>
		...
		<input type="text" class="form-control" name="name" id="order-name" value="{if isset($name)}{$name|escape}{/if}" placeholder="{$lang->enter_your_name}" required>
				<div class="invalid-feedback">{$lang->enter_your_name}</div>
			</div>
			<div class="mb-3">
				<label for="order-email">Email</label>
				<input type="text" class="form-control" name="email" id="order-email" value="{if isset($email)}{$email|escape}{/if}" placeholder="{$lang->enter_your_email}" required>
				<div class="invalid-feedback">{$lang->enter_your_email}</div>
			</div>
			<div class="mb-3">
				<label for="order-phone">{$lang->phone}</label>
				<input type="text" class="form-control" name="phone" id="order-phone" value="{if isset($phone)}{$phone|escape}{/if}" placeholder="{$lang->enter_phone_number}">
			</div>
			<div class="mb-3">
				<label for="order-address">{$lang->delivery_address}</label>
				<input type="text" class="form-control" name="address" id="order-address" value="{if isset($address)}{$address|escape}{/if}" placeholder="{$lang->enter_the_address}">
			</div>
			<div class="mb-3">
				<label for="order-comment">{$lang->comment}</label>
				<textarea class="form-control" name="comment" id="order-comment" placeholder="{$lang->enter_a_comment}" rows="4">{if isset($comment)}{$comment|escape}{/if}</textarea>
