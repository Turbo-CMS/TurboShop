		...
		<div class="fix-height">
			...
			<div class="mt-1">
				...
				{if $product->variant->stock}
					<div class="text-success mt-1"><i class="fal fa-circle-check me-1"></i>{$lang->In_stock}</div>
				{else}
					<div class="text-danger mt-1"><i class="fa-light fa-circle-xmark me-1"></i>{$lang->not_available}</div>
				{/if}
			</div>
		</div>
		<div class="btn-toolbar justify-content-between mt-0">
			...
			<div class="btn-group" role="group" aria-label="cardFooterButton">
				<button class="btn btn-link product-card-quickview" type="button" data-url="{$lang_link}products/{$product->url}" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="{$lang->quick_view}"><i class="fal fa-eye text-muted"></i></button>
				...
				{else}
					<a class="btn btn-link wishlist" href="{$lang_link}wishlist/{$product->url}" data-result-text="{$lang->added_to_wishlist}" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="{$lang->add_to_wishlist}"><i class="fal fa-heart text-muted"></i></a>
				{/if}
				...
				{else}
					<a class="btn btn-link compare" href="{$lang_link}compare/{$product->url}" data-result-text="{$lang->added_to_compare}" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="{$lang->add_to_compare}"><i class="fal fa-sync text-muted"></i></a>
				{/if}
			</div>
		</div>
	</div>
