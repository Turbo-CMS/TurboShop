					...
					<div class="action-wrap">
						<form class="variants" action="{$lang_link}cart">
							...
							{if $product->variant->stock}
								<div class="text-success"><i class="fal fa-circle-check me-1"></i>{$lang->In_stock|escape}</div>
							{else}
								<div class="text-danger"><i class="fa-light fa-circle-xmark me-1"></i>{$lang->not_available|escape}</div>
							{/if}
							<div class="d-grid gap-2 d-block mt-3">
								{if $product->variants|count> 1}
									<a class="btn btn-secondary" href="{$lang_link}products/{$product->url}" role="button"><i class="fal fa-external-link me-2"></i>{$lang->choose|escape}</a>
								{else}
									<button type="submit" data-result-text="<i class='fal fa-shopping-bag me-2'></i>{$lang->added_cart|escape}" class="btn btn-primary" value="{$lang->add_cart|escape}" title="{$lang->add_cart|escape}" {if !$product->variant->stock}disabled{/if}><i class="fal fa-shopping-bag me-2"></i>{$lang->add_cart|escape}</button>
								{/if}
								<a href="{$lang_link}products/{$product->url}" class="btn btn-secondary" role="button">{$lang->more_details|escape}<i class="fal fa-chevron-right ms-2"></i></a>
							</div>
						</form>
						<div class="btn-group mt-2" role="group" aria-label="Card list button">
							<button class="btn btn-link product-card-quickview" type="button" data-url="{$lang_link}products/{$product->url}" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="{$lang->quick_view|escape}"><i class="far fa-eye text-muted"></i></button>
							{if $wishlist}
								<a class="btn btn-link" href="{$lang_link}wishlist/remove/{$product->url}" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="{$lang->delete|escape}"><i class="fal fa-trash text-muted"></i></a>
							{elseif $wishlist_products && in_array($product->url, $wishlist_products)}
								<a class="btn btn-link" href="{$lang_link}wishlist" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="{$lang->added_to_wishlist|escape}"><i class="fal fa-heart text-danger"></i></a>
							{else}
								<a class="btn btn-link wishlist" href="{$lang_link}wishlist/{$product->url}" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="{$lang->add_to_wishlist|escape}"><i class="fal fa-heart text-muted"></i></a>
							{/if}
							{if isset($smarty.session.compared_products) && in_array($product->url, $smarty.session.compared_products)}
								<a class="btn btn-link" href="{$lang_link}compare" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="{$lang->added_to_compare|escape}"><i class="fal fa-sync text-primary"></i></a>
							{else}
								<a class="btn btn-link compare" href="{$lang_link}compare/{$product->url}" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="{$lang->add_to_compare|escape}"><i class="fal fa-sync text-muted"></i></a>
							{/if}
						</div>
					</div>
				</div>
