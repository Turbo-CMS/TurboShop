...
{if $posts}
	<div class="btn-toolbar justify-content-between my-4" role="toolbar" aria-label="Articles sort">
		...
		{else}
			<h1>{if isset($articles_category->name_h1)}{$articles_category->name_h1|escape}{elseif isset($articles_category->name)}{$articles_category->name|escape}{/if}</h1>
		{/if}
		...
		{foreach $posts as $post}
			<div class="col-md-12 col-lg-6">
				...
				<div itemscope itemtype="http://schema.org/Article" class="card mb-4">
					...
					{/if}
					<div class="card-body">
						<a href="{$lang_link}article/{$post->url}" class="text-decoration-none card-title">
							<h2 data-article="{$post->id}" itemprop="name headline" class="h5">{$post->name|escape}</h2>
						</a>
						...
						<div class="btn-group" role="group" aria-label="Article group">
							<a class="btn btn-link text-muted" href="{$lang_link}article/{$post->url}"><i class="far fa-comment"></i><span class="badge text-muted text-decoration-none">{$post->comments_count}</span></a>
