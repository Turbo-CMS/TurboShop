<div class="mobile-offcanvas col-lg-3 bg-white" id="sidebar">
                                ...
                                {if $cat->visible}
                                    <span class="hidden-sm-down list-group-item {if $articles_category->id == $cat->id}bg-primary{/if}">
                                        ...
                                        {if $cat->subcategories}
                                            <a data-toggle="collapse" href="#menusub{$cat->id}" {if $articles_category->id == $cat->id || in_array($articles_category->id, $cat->children)}aria-expanded="true" {/if}>
                                                <i class="far fa-angle-down"></i>
                                            </a>
                                        {/if}
                                    </span>
			...
			{foreach $categories as $c}
				{if $c->visible}
					<span class="hidden-sm-down list-group-item {if $category->id == $c->id}bg-primary{/if}">
						...
						{if $c->subcategories}
							<a data-toggle="collapse" data-parent="#sidebar" href="#menu{$c->id}" {if $category->id == $c->id || in_array($category->id, $c->children)}aria-expanded="true" {/if}>
								<i class="far fa-angle-down"></i>
							</a>
						{/if}
					</span>
							...
							{foreach $c->subcategories as $cat}
								{if $cat->visible}
									<span class="hidden-sm-down list-group-item {if $category->id == $cat->id}bg-primary{/if}">
										...
										{if $cat->subcategories}
											<a data-toggle="collapse" aria-expanded="false" href="#menusub{$cat->id}" {if $category->id == $cat->id || in_array($category->id, $cat->children)}aria-expanded="true" {/if}>
												<i class="far fa-angle-down"></i>
											</a>
										{/if}
									</span>
				...
				{/if}
			{/foreach}
			<span class="hidden-sm-down list-group-item bg-light"><a href="{$lang_link}all-products">{$lang->all_products}</a></span>
		</div>
	{/if}
	...
	{if $currencies|count>1}
		<div class="card my-4">
			...
			<div class="card-body">
				<select class="form-select" onchange="location.href=this.value" size="1" aria-label="Default select example">
							...
							{foreach $c->subcategories as $cat}
								{if $cat->visible}
									<span class="hidden-sm-down list-group-item {if $category->id == $cat->id}bg-primary{/if}">
										...
										{if $cat->subcategories}
											<a data-toggle="collapse" href="#menusub{$cat->id}" {if $category->id == $cat->id || in_array($category->id, $cat->children)}aria-expanded="true" {/if}>
												<i class="far fa-angle-down"></i>
											</a>
										{/if}
									</span>
