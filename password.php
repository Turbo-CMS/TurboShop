		...
		if ($c = $turbo->request->get('code')) {
			...
			if ($newPassword = $turbo->request->post('new_password')) {
				...
				if (!is_writable($passwdFile)) {
					echo '<h1>' . $turbo->backendTranslations->administrator_password_recovery . '</h1>';
					echo '<p class="error">' . $turbo->backendTranslations->file_is_not_writable . '</p>';
					echo '<p>' . $turbo->backendTranslations->file_is_not_writable_message . '</p>';
				} else {
					$newLogin = $turbo->request->post('new_login');
					$newPassword = $turbo->request->post('new_password');
					if (!$turbo->managers->updateManager($newLogin, ['password' => $newPassword])) {
						$turbo->managers->addManager(['login' => $newLogin, 'password' => $newPassword]);
					}
					echo '<h1>' . $turbo->backendTranslations->administrator_password_recovery . '</h1>';
					echo '<p class="success">' . $turbo->backendTranslations->new_password_set . '</p>';
					echo '<p><a href="' . $turbo->root_url . '/turbo/index.php?module=ManagersAdmin">' . $turbo->backendTranslations->go_to_control_panel . ' &#129122;</a></p>';
				}
			} else {
				echo '<h1>' . $turbo->backendTranslations->administrator_password_recovery . '</h1>';
				echo '<form method="post">';
				echo '<label class="form-label">' . $turbo->backendTranslations->new_login . '</label><br>';
				echo '<input type="text" name="new_login"><br>';
				echo '<label class="form-label">' . $turbo->backendTranslations->new_password . '</label><br>';
				echo '<input type="password" name="new_password"></label><br>';
				echo '<input class="mt-1" type="submit" value="' . $turbo->backendTranslations->save_login_and_password . '">';
				echo '</form>';
			}
		} else {
			echo '<h1>' . $turbo->backendTranslations->administrator_password_recovery . '</h1>';
			echo '<form method="post" action="' . $turbo->root_url . '/password.php">';
			echo '<label class="form-label">' . $turbo->backendTranslations->enter_admin_email . '</label><br>';
			echo '<input type="text" name="email">';
			echo '<input class="mt-h" type="submit" value="' . $turbo->backendTranslations->restore_password . '">';
			echo '</form>';
			...
			if (isset($_POST['email'])) {
				...
				echo '<p class="success">' . $turbo->backendTranslations->message_password_recovery_4 . '</p>';
			}
		}
		?>
	</div>
