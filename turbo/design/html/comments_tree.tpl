	...
	<div class="col-lg-8 col-md-8">
		<div class="d-md-flex mb-3">
			<h1 class="d-inline align-middle me-3">
				...
				{elseif $type=='product'}
					{$btr->global_comments} {$btr->comments_to_products|escape} - {$comments_count}
				{elseif $type=='blog'}
												...
												<div class="turbo-list-boding turbo-list-comments-name {if $level > 0}admin-note{/if}">
													...
													{if $level == 0}
														...
														{if $comment->type == 'product'}
															{$btr->comments_product|escape} <a target="_blank" href="{$config->root_url}/products/{$comment->product->url}#comment_{$comment->id}" class="fw-bold text-body text-decoration-none">{$comment->product->name}</a>
