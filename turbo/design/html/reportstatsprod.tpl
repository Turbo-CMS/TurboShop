					...
					<div class="row">
						{if $variants|count > 1}
							<div class="col-md-3 col-lg-3 col-sm-12 mb-3">
								<select class="selectpicker" data-live-search="true" data-size="10" onchange="location = this.value;">
									<option value="{url keyword=null brand_id=null page=null limit=null variant_id=null}" {if !$variant_id}selected{/if}>{$btr->variants_all|escape}</option>
						...
						<div class="col-md-3 col-lg-3 col-sm-12 mb-3">
							<select class="selectpicker" data-live-search="true" data-size="10" onchange="location = this.value;">
								...
								<option value="{url status=1}" {if $status==1}selected{/if}>{$btr->global_new_order|escape}</option>
								<option value="{url status=2}" {if $status==2}selected{/if}>{$btr->global_accepted_order|escape}</option>
								<option value="{url status=3}" {if $status==3}selected{/if}>{$btr->global_closed_order|escape}</option>
								<option value="{url status=4}" {if $status==4}selected{/if}>{$btr->global_canceled_order|escape}</option>
							</select>
						</div>
						<div class="col-md-3 col-lg-3 col-sm-12 mb-3">
							<select class="selectpicker" data-live-search="true" data-size="10" onchange="location = this.value;">
		...
		<div class="row">
			<div class="col-12">
				<div class="text-dark fw-bold text-end mt-3">
					<div class="h3">{$btr->global_total|escape} {$total_summ|string_format:'%.2f'} {$currency->sign|escape} <span class="text-secondary">({$total_amount} {$btr->reportstats_units|escape})</span></div>
				</div>
			</div>
		</div>
		<div class="row">
