	...
	<div class="col-lg-7 col-md-7">
		<div class="wrap_heading">
			...
			<div class="box_btn_heading">
				<a class="btn btn_small btn-primary" href="{url module=CategoryAdmin return=$smarty.server.REQUEST_URI}">
								...
								{if $level > 1}
									...
									<div class="fn_row {if $level == 1}turbo_list_body_item{/if} fn_sort_item body_narrow row_narrow">
										<div class="turbo_list_row {if $level > 1}fn_sort_item{/if} narrow">
											...
											<div class="turbo_list_boding turbo_list_photo turbo_list_category_photo hidden-sm-down small_photo boding_small">
												{if $category->image}
											...
											<div class="turbo_list_boding turbo_list_status">
												...
												<div class="form-check form-switch">
													<input class="form-check-input fn_ajax_action {if $category->visible}fn_active_class{/if}" id="id_{$category->id}" data-module="category" data-action="visible" data-id="{$category->id}" name="visible" value="1" type="checkbox" {if $category->visible}checked="" {/if}>
													<label class="form-check-label" for="id_{$category->id}"></label>
												</div>
											</div>
											<div class=" turbo_list_setting">
												{* open *}
												<a href="../{$lang_link}catalog/{$category->url}" target="_blank" data-hint="{$btr->general_view|escape}" class="setting_icon setting_icon_open hint-bottom-middle-t-info-s-small-mobile  hint-anim">
													{include file='svg_icon.tpl' svgId='icon_desktop'}
												</a>
												{*featured*}
												<button data-hint="{$btr->categories_featured|escape}" type="button" class="setting_icon setting_icon_featured fn_ajax_action {if $category->featured}fn_active_class{/if} hint-bottom-middle-t-info-s-small-mobile  hint-anim" data-module="category" data-action="featured" data-id="{$category->id}">
