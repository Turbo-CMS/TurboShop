	...
	private function import_item($item)
	{
		...
		if ((empty($product_id) || empty($variant_id)) && isset($item['name'])) {
			...
			elseif (isset($item['variant']))
				$this->db->query('SELECT v.id as variant_id, p.id as product_id FROM __products p LEFT JOIN __variants v ON v.product_id=p.id AND v.sku=? WHERE p.name=? LIMIT 1', $item['variant'], $item['name']);
			else
