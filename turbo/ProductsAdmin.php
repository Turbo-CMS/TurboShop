		...
		if ($this->request->method('post')) {
			...
			if (!empty($ids))
				switch ($this->request->post('action')) {
					...
					case 'set_is_hit': {
							$this->products->update_product($ids, array('is_hit' => 1));
							break;
						}
					case 'unset_is_hit': {
							$this->products->update_product($ids, array('is_hit' => 0));
							break;
						}
					case 'set_export': {
							$this->products->update_product($ids, array('to_export' => 1));
							break;
						}
					case 'unset_export': {
							$this->products->update_product($ids, array('to_export' => 0));
							break;
						}
					case 'delete': {
							foreach ($ids as $id)
					...
					case 'move_to_page': {
							...
							// We will open this page soon
							$filter['page'] = $target_page;

							// Which item to move to
							...
							// If we climbed out for the last product, we take the position of the last product as the target of the movement
					...
					case 'change_price': {
							...
							foreach ($this->variants->get_variants(array('product_id' => $ids)) as $variant) {
								...
								elseif ($operation == 1 && $value != 0) $price = $price * (1 + $value / 100);
					...
					case 'move_to_brand': {
							...
							// Re-select brands in the category
							$brands = $this->brands->get_brands(array('category_id' => $category_id));
							$this->design->assign('brands', $brands);
